  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        const response = await axios.get(`https://gaming-hustler-server.onrender.com/users?email=${user.email}`);
        const existingUser = response.data.length > 0;

        if (!existingUser) {
          // If the user doesn't exist, create a new user
          await axios.post('https://gaming-hustler-server.onrender.com/new-user', userImp);
        }
